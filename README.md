...
- 免费流量：支持通过运行或打包配置提供免费流量入口，按周限制可用流量，用完后自动停止使用该入口。
...
```text
dist/wing-1.0.5.2-windows-x64-setup.exe
```
...
./scripts/mk.ps1 package # 构建并生成 dist/wing-1.0.5.2-windows-x64-setup.exe 标准安装包
...
- `dist/wing-1.0.5.2-windows-x64-setup.exe`：标准 Windows 安装包，用户双击后可选择安装目录。
...
```bash
WING_VERSION=1.0.5.2 FLUTTER_BUILD_NUMBER=10052 bash scripts/package-android.sh
```
...
- `wing-1.0.5.2-windows-x64-setup.exe`
- `wing-1.0.5.2-linux-x64.run`
- `wing-1.0.5.2-macos-x64.pkg`
- `wing-1.0.5.2-android-universal.apk`
- `wing-1.0.5.2-ios-unsigned.ipa`
