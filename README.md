...
wing 是一款面向 Windows 的桌面代理客户端。它集成 sing-box、Mieru Client、本地 HTTP 代理服务与 Web 控制面板，提供节点订阅、测速、网站可用性测试、自动选点、规则分流、网络共享、隧道连接、DNS 分流、WebRTC 防泄漏和命令行进程规则等能力。

## 核心特性

- 多协议支持：Hysteria2、TUIC、VLESS、VMess、Trojan、Shadowsocks、AnyTLS、Naive、Mieru、HTTP/SOCKS 等。
- 双入口管理：Wails 桌面窗口用于日常操作，系统托盘用于快速切换节点、代理模式、网络共享、隧道连接和退出。
...
- 网络共享：可将本机 HTTP 代理监听切换到局域网地址，让同一网络中的设备使用当前节点。
- 隧道连接：基于 Wintun + tun2socks 接管不遵循系统代理设置的应用流量；需要管理员权限。
...
2. 在桌面窗口或托盘菜单中打开控制面板。
3. 导入订阅链接或节点分享链接。
4. 选择节点并开启系统代理；需要给局域网设备使用时开启网络共享，需要接管更多应用流量时开启隧道连接。
