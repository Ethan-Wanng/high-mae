...
wing 是一款面向 Windows 的桌面代理客户端。它集成 sing-box、Mieru Client、本地 HTTP 代理服务与 Web 控制面板，提供节点订阅、测速、网站可用性测试、自动选点、规则分流、TUN 接管、DNS 分流、WebRTC 防泄漏和命令行进程规则等能力。
...
- 自动选点：支持全节点、当前订阅组和指定聚合组范围内按延迟自动选择节点；开启后每 5 分钟自动重测候选节点并选择一次，也支持手动立即选择。
- 自动选点规则：用户可维护排除关键字、只选地区、只选节点、只选订阅组、只选聚合组和网站可用性规则；默认提供可删除的“不使用香港的节点”规则。
