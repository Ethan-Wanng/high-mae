...
- Android 是独立原生客户端而非网页套壳：APK 内置 Go 代理核心，使用 Flutter 原生页面管理连接、节点、订阅切换/编辑/分享、自动选优、测速、网站测试、DNS/路由编辑与流量状态；系统 `VpnService` 会显示授权说明、常驻通知和 VPN 状态图标，并在 Go 核心确认接管 TUN 文件描述符后才报告连接成功。通用包强制包含 arm64-v8a、armeabi-v7a 与 x86_64（模拟器）核心。
- Android 稳定性与性能：TUN 改用带 gVisor 的移动端用户态网络栈；节点操作使用订阅文件与组内位置稳定寻址，核心重启后不会复用失效索引；各分页按需加载，首页仅轮询轻量流量摘要，并增加带正式 Logo 的启动画面。
...
- 多入口管理：Flutter 桌面窗口启动后自动显示，Go 系统托盘用于再次唤起窗口、快速切换代理模式、隧道连接和退出；Android 使用独立原生客户端，iOS 暂保留本地控制面板入口。
- 灵动岛导航：默认以收缩胶囊呈现，鼠标悬停后展开为纯图标标签；当前网络状态会改变界面配色，首屏以居中的节点入口卡片作为默认操作入口。
- 本地控制面板：默认运行在 `http://127.0.0.1:10809/`，用于订阅导入、节点管理、测速、网站测试、规则管理、DNS 管理、帮助中心和流量统计。
- 节点管理器：订阅组、聚合组、自定义节点、自动选择和自动节点集中在同一入口；“添加节点”位于“自定义节点”子标签，“导入订阅”位于“订阅组”子标签。
- 订阅管理：支持导入订阅、编辑订阅链接、调整自动更新时间间隔、分享订阅源、刷新订阅，并使用唯一订阅文件名避免多个订阅组冲突。
- 延迟测速：支持单节点、订阅组和聚合组测速；展开订阅组或聚合组时会自动测试组内节点延迟，节点管理器可一键忽略 Timeout 节点。
- 自动选点：支持全节点、指定订阅组和指定聚合组范围内按延迟自动选择节点；开启后会按规则和节点变化自动选择，也支持手动立即选择。
...
节点管理器现在以来源子标签组织：订阅组用于导入、编辑、刷新和删除订阅；聚合组用于从多个订阅中组合节点；自定义节点用于添加单节点链接；自动选择用于维护候选范围和规则。节点来源标签下方的“忽略超时节点”会隐藏当前所有来源列表中延迟为 Timeout 的节点，适合测速后快速清理视图。

订阅组的“编辑”会同时保存订阅链接和自动更新时间。订阅地址变更时无需删除旧组，直接编辑原订阅组即可保留它在界面中的位置和关联习惯。
...
3. 在“节点管理 / 订阅组”导入订阅；单节点链接请到“节点管理 / 自定义节点”添加。
4. 订阅地址或更新时间变化时，在订阅组详情里点击“编辑”保存新地址和更新间隔。
5. 测速后如需隐藏 Timeout 节点，打开节点来源标签下方的“忽略超时节点”。
6. 选择节点并开启系统代理；需要接管更多应用流量时开启隧道连接。
7. 在网站测试中检查当前节点对常用网站的支持情况。
8. 在节点管理的自动选择标签中按需开启自动选点，并维护自动选点过滤规则。
9. 在规则管理中配置域名规则或命令行进程规则。
