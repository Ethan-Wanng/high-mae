...
// SetOnClick registers a callback for a primary click on the tray icon.
func SetOnClick(onClick func()) {
	systrayClickMu.Lock()
	defer systrayClickMu.Unlock()
	systrayClick = onClick
}

func notifySystrayClick() {
	systrayClickMu.RLock()
	onClick := systrayClick
	systrayClickMu.RUnlock()
	if onClick == nil {
		return
	}
	go func() {
		defer func() {
			if err := recover(); err != nil {
				log.Errorf("systray click callback panicked: %v", err)
			}
		}()
		onClick()
	}()
